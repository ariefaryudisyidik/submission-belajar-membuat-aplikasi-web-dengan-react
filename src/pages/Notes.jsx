...
export default function Notes() {
  const [notes, setNotes] = useState(
    notesList.filter((note) => !note.archieved)
  );
  ...
  const showArchievedHandler = () =>
    setShowArchieved((prev) => {
      setSearching(false);
  ...
  return (
    <div className="container">
      ...
      <h2 className="title">
        {showArchieved ? 'Daftar Arsip Catatan' : 'Daftar Catatan'}
      </h2>
      <div id="archieved">
        <button onClick={showArchievedHandler}>
          Tampilkan {showArchieved ? 'Catatan' : 'Arsip Catatan'}
        </button>
      </div>
      ...
      {searching ? (
        searchResult.length === 0 ? (
          <h2 className="title">
            Tidak ada {showArchieved ? 'arsip catatan' : 'catatan'} yang sesuai
          </h2>
        ) : (
        ...
        archieved.length === 0 ? (
          <h2 className="title">Belum ada arsip catatan</h2>
        ) : (
          archieved.map((note) => (
            <NoteCard
              key={note.id}
        ...
        <h2 className="title">Belum ada catatan</h2>
      ) : (
        notes.map((note) => (
          <NoteCard
            key={note.id}
